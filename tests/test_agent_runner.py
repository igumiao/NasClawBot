...
class FakeDownloadAutomation:
    """Mock download automation that records submission requests.
    ...
    def submit_downloads(
        ...
        items = [self._submit(request) for request in requests]
        return BatchDownloadSubmissionResult(items=items)

    def _submit(self, request: Any) -> DownloadSubmissionResult:
...
def test_nasclaw_agent_runner_persists_and_restores_checkpoint(tmp_path, monkeypatch: pytest.MonkeyPatch):
    ...
    assert second.checkpoint.metadata["tool_names"] == sorted([
        ...
        "memory_search",
        "mteam_search",
        # Download-add tools are registered only when automation is configured.
...
def test_nasclaw_agent_runner_gates_download_tool_by_default(tmp_path, monkeypatch: pytest.MonkeyPatch):
        ...
        LLMToolResponse(
            ...
            tool_calls=[
                ToolCall(
                    id="call-download",
                    name="qb_add_torrent",
                        arguments='{"torrent_id":"123","qb_category":"movie","completion_action":"notify"}',
                )
            ],
            model="fake-model",
        ),
    ...
    runner = NasClawAgentRunner(
        checkpoint_store=store,
        download_automation_factory=lambda: FakeDownloadAutomation(),
    )
    ...
    assert result.pending_approvals[0]["arguments"] == {
        "torrent_id": "123", "qb_category": "movie",
        "completion_action": "notify", "save_path": "/tmp/test-downloads",
    }
...
def test_nasclaw_agent_runner_approve_can_pause_for_next_gated_tool(tmp_path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(agent_runner, "get_settings", lambda: FakeSettings())
    FakeQBAdapter.calls = []
    FakeLLM.calls = []
    FakeLLM.tool_choices = []
    ...
    pending = runner.run("session-resume-next-approval", "下载后恢复")

    result = runner.approve("session-resume-next-approval", pending.pending_approvals[0]["approval_id"])
