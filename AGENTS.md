...
- `app/runtime/`: durable background task system. `RuntimeTaskStore` (SQLite persistence with `BEGIN IMMEDIATE` for atomic claim), `TaskScheduler` (external API), `TaskWorker` (in-process async loop with per-kind semaphores, lease-based claim, exponential backoff retry), `HandlerRegistry` (kind-to-handler mapping). Two handlers: `download_watch` (qB completion polling with dynamic ETA-based intervals via EMA-smoothed speed, warm-up gating on downloading states only, hash resolution with max attempts), `organize_download` (file organization via `OrganizeWorkerAgent`). Task lifecycle: `INITIALIZING → QUEUED → RUNNING → WAITING (Reschedule) or terminal (SUCCEEDED/FAILED/CANCELLED)`. `max_failure_attempts=8` (configurable); `failure_count` tracks retry exhaustion separately from `attempts` (observability). Retry uses exponential backoff (30s→60s→120s… capped at 3600s). Terminal tasks auto-purged after `TASK_PURGE_MAX_AGE_SECONDS` (default 300s) — deletes tasks + runs only, NOT events. Event lifecycle is independent: no FK on `runtime_task_events.task_id`, two-tier purge (consumed events: acknowledged + injected, both >`EVENT_CONSUMED_PURGE_SECONDS` default 3600s; absolute max: `EVENT_MAX_AGE_SECONDS` default 604800s).
...
The runtime task system provides post-download automation: `download_watch` tasks poll qB for completion (dynamic ETA-based intervals via EMA-smoothed speed, half-ETA polling clamped 30s–600s, warm-up gated on downloading states only), then spawn `organize_download` tasks or emit `download_completed` events per the follow-up mode. `organize_download` tasks run a constrained `ToolCallingAgent` (10 tools, dynamic Gate gated on skill load) via `OrganizeWorkerAgent` to move files into the media library. Task events (`download_completed`, `organize_completed`, `task_failed`) are independent from task lifecycle — no FK on `runtime_task_events.task_id`. Events are injected into the Agent system prompt on each turn so the LLM is aware of background activity, and purged on a two-tier policy: consumed events (acknowledged + injected, both >1h), or absolute max age (>7d). The `OperationJournal` (`app/runtime/organize_journal.py`) is reserved for V2 idempotent retry reconciliation.
