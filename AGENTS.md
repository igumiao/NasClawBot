...
- `POST /chat/agent/sessions/{session_id}/approvals/{approval_id}/approve`: deterministically executes an approved pending `qb_add_torrent` call, updates checkpoint metadata, and lets the LLM produce a no-tools final summary from safe receipt fields. If summary fails, it falls back to a deterministic receipt message. It does not resume the provider tool-call loop.
