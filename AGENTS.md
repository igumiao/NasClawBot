...
- `app/tools/`: per-tool modules (`current_time.py`, `memory_search.py`, `remember_this.py`, `mteam_search.py`, `tavily_search.py`, `member_profile.py`, `qb_add_torrent.py`, `qb_list_torrents.py`, `qb_get_torrent.py`, `qb_list_tags.py` (active), `qb_list_categories.py` (deprecated, kept for compat), `qb_control_torrent.py`, `qb_set_global_speed.py`, `qb_set_torrent_speed.py`, and TMDB tools), re-exported via `__init__.py`. `qb_add_torrent.py` handles both single and batch submission (the former `qb_add_torrents.py` was merged into it).
...
- `metadata["authorization_grants"]`: session-scoped download-add grants created by approving with `approve_and_grant_session`. Grants are checked before `Gate` for `qb_add_torrent` only. `completion_action=organize`, monitor create/update, and task cancel always require one-time approval.
