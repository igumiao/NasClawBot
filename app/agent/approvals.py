...
class ApprovalRecord:
    ...
    def from_dict(cls, data: dict[str, Any], session_id: str | None = None) -> "ApprovalRecord":
        ...
        expires_at = str(data.get("expires_at") or (datetime.now() + APPROVAL_TTL).isoformat())
    ...
    def is_expired(self, now: datetime | None = None) -> bool:
        return (now or datetime.now()) > _parse_datetime(self.expires_at)
...
def create_pending_approval(
    ...
    created_at = str(raw.get("created_at") or (now or datetime.now()).isoformat())
...
def mark_approved(
    ...
    record.decided_at = (now or datetime.now()).isoformat()
...
def mark_expired(
    ...
    record.expired_at = (now or datetime.now()).isoformat()
